let jsondataMDS_EucData = "";
let apiUrlmds_euc = "http://127.0.0.1:5000/getmds_euc"
...
async function drawMds_euc(data3) {
    ...
    jsondataMDS_EucData = await fetchJsonData(apiUrlmds_euc)
    //console.log(jsondataMDS_EucData)
    data1 = jsondataMDS_EucData["xval"]
    data2 = jsondataMDS_EucData["yval"]


    var myDATA = [];
    console.log(Math.max.apply(Math, data1))
    ...
    svg.on("mousedown", null)
     ...
     var y = d3.scaleLinear().range([height, 0]).domain([d3.min(data2), d3.max(data2)+1]);
